<x-app-layout>
    <x-breadcrumbs name="citation.edit" :data="$citation" />
    <h1 class="font-semibold text-lg my-8">Ubah Sitasi</h1>
    ...
    <div class="w-1/2">
        ...
        <x-card-container>
            <form action="{{ route('admin.citation.update', $citation->id) }}" method="POST">
                @csrf
                @method('PUT')
                <x-input id="title" name="title" label="Judul Sitasi" type="text" :value="$citation->title" class="mb-2" required />
                <x-select id="author" name="author" label="Penulis" isFit="" required>
                    <option value="">Pilih Penulis</option>
                    @foreach ($authors as $author)
                        <option value="{{ $author->id }}" {{
                            $citation->author_id == $author->id ? 'selected' : ''
                        }}>{{ $author->name }}</option>
                    @endforeach
                </x-select>
